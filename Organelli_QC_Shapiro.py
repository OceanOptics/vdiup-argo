...
def organelli16_qc(df: object, lat: object = float('nan'), lon: object = float('nan'),
    ...
    # remove rows with depth NaN values
    ...
    for qc_wl in qc_wls:
        try:
            ...
            # Because sometimes the value are non-monotomous, we are removing the values BELOW the idx of the Zdark,
            # regardless if the float comes back up after (i.e. if some values below are ABOVE the Zdark
            df_filtered = df.loc[:p0_index, :]
            ...
            if len(depth_valid) == 0 or len(log_rad_valid) == 0 or len(depth_valid) != len(log_rad_valid):
                flags[:] = bad
                results.append((2, flags, 'BAD: INVALID INPUT DATA FOR POLYNOMIAL FIT', False, qc_wl))
            else:
                try:  # Fit polynomial
                    ...
                    flags[:] = bad
                    results.append((2, flags, 'BAD: POLYNOMIAL FIT FAIL', False, qc_wl))
