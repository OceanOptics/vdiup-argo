...
def solar_zenith_and_azimuth_angle(longitude: float,
    ...
    day = time_utc.day
    day_hours = (time_utc.hour + time_utc.minute / 60.0
    ...
    lat_rad = np.deg2rad(np.array([latitude]))
    lon_rad = np.deg2rad(np.array([longitude]))
    ...
    r_asc = np.array([np.arctan2(sl * ce, cl)])
